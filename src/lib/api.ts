...
// ─── Microsoft Auth — Email ↔ Account Linking ────────────────────────────────
...
const TEACHER_PREFIXES = ['Mr.', 'Mrs.', 'Ms.', 'Miss', 'Dr.', 'Coach', 'Prof.']

/**
 * Looks up a player profile by Microsoft email using a secure database
 * function (never exposes the email column via a broad SELECT).
 * Returns the parsed account info, or null if not found.
 */
export const lookupAccountByEmail = async (
  ...
  try {
    ...
    const gradeStr = String(row.grade)
...
/**
 * Writes the Microsoft email into the player's profile row so they can be
 * found by email on future logins from any device.
...
export const linkEmailToCurrentAccount = async (email: string): Promise<void> => {
  ...
  const normalizedGrade = normalizeLegacyGrade(rawGrade)
  const playerKey = buildPlayerStateKey(displayName, normalizedGrade)
  const safeEmail = email.toLowerCase().trim()

  try {
    // Upsert so this works even if the profile row hasn't been created yet.
    await supabase.from(SUPABASE_TABLES.playerProfiles).upsert(
      {
        player_key: playerKey,
        player_name: sanitizePlayerName(displayName),
        player_name_key: normalizeNameKey(displayName),
        grade: Number(normalizedGrade) || 0,
        microsoft_email: safeEmail,
        updated_at: new Date().toISOString(),
      },
      { onConflict: 'player_key' }
    )
  } catch {
    // Best-effort — never block gameplay on linking failures.
  }
}
